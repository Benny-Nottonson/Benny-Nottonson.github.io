...
export class Circle {
  ...
  public update(
    ...
    const pos = {
      x: this.position.x + this.translate.x,
      y: this.position.y + this.translate.y,
    };
    ...
    const edge = Math.min(
      pos.x - this.size,
      width - pos.x - this.size,
      pos.y - this.size,
      height - pos.y - this.size,
    );
    const offset = 50 / this.magnetism;

    this.alpha =
      edge > 1
        ? Math.min(this.alpha + 0.02, this.targetAlpha)
        : this.targetAlpha * edge;
